...
module Tools
  class ListUserPullRequestActivity
    ...
    warn "Environment Variable GITHUB_USERNAME is not set" if @username.nil?
    @organization = ENV["GITHUB_ORGANIZATION"]
    warn "Environment Variable GITHUB_ORGANIZATION is not set" if @organization.nil?
    @personal_access_token = ENV["GITHUB_PERSONAL_ACCESS_TOKEN"]
    warn "Environment Variable GITHUB_PERSONAL_ACCESS_TOKEN is not set" if @personal_access_token.nil?
